...
-- Policies: owner can CRUD their runs; service role bypasses RLS
create policy if not exists "Users can view their own runs" on public.automation_runs
for select using (auth.uid() = user_id);

create policy if not exists "Users can insert their own runs" on public.automation_runs
for insert with check (auth.uid() = user_id);

create policy if not exists "Users can update their own runs" on public.automation_runs
for update using (auth.uid() = user_id);

create policy if not exists "Users can delete their own runs" on public.automation_runs
...
create policy if not exists "Users can view their own run events" on public.automation_events
